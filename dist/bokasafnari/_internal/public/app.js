...
bookPublisherInput.addEventListener('input', () => {
    saveAutoSession();
});
...
function setupDragAndDrop(el) {
    ...
    el.addEventListener('drop', function (e) {
        ...
        if (dragSrcEl !== this) {
            pushUndoState('Reorder Chapters');
...
function updateFontSizeUI(notify = true) {
...
// Ctrl+Z to undo, Ctrl+S to save session, Ctrl+O to open, Ctrl+H for find/replace, Escape to close
document.addEventListener('keydown', (e) => {
    if (e.ctrlKey && e.key.toLowerCase() === 'z') {
        const activeEl = document.activeElement;
        const isEditingField = activeEl && (activeEl.tagName === 'INPUT' || activeEl.tagName === 'TEXTAREA' || activeEl.isContentEditable);
        if (!isEditingField && state.undoStack.length > 0) {
            e.preventDefault();
            undo();
        }
    }
...
function replaceAll() {
    if (frMatches.length === 0) return;
    pushUndoState('Replace All Matches');
    const replacement = frReplaceInput.value;
...
// ═══════════════════════════════════════════════════════════════
// SESSION MANAGEMENT (SAVE / OPEN / AUTOSAVE)
// ═══════════════════════════════════════════════════════════════

const AUTOSAVE_KEY = 'bokasafnari_autosave_session';
...
function loadSessionData(sessionData, filename = null) {
    if (!sessionData || typeof sessionData !== 'object') return false;

    const metadata = sessionData.metadata || {};
    bookTitleInput.value = metadata.title || 'My Web Collection';
    bookAuthorInput.value = metadata.author || 'Scraped Reader';
    bookPublisherInput.value = metadata.publisher || 'bokasafnari';

    coverTitlePreview.innerText = bookTitleInput.value;
    coverAuthorPreview.innerText = bookAuthorInput.value;

    state.chapters = Array.isArray(sessionData.chapters) ? sessionData.chapters : [];
    state.fontSize = sessionData.fontSize || 'md';
    state.undoStack = [];
    if (undoBtn) undoBtn.disabled = true;
    updateFontSizeUI(false);

    const loadedIdx = sessionData.selectedChapterIndex;
    if (typeof loadedIdx === 'number' && loadedIdx >= 0 && loadedIdx < state.chapters.length) {
        state.selectedChapterIndex = loadedIdx;
    } else {
        state.selectedChapterIndex = state.chapters.length > 0 ? 0 : null;
    }
    ...
    if (filename) {
        showToast(`Session loaded: ${filename}`);
    }
    return true;
}
...
function saveAutoSession() {
    try {
        ...
        console.warn('Failed to save autosave to localStorage:', e);
    }
}
...
function loadAutoSession() {
    try {
        ...
        if (saved) {
            ...
            if (parsed && (parsed.chapters?.length > 0 || (parsed.metadata?.title && parsed.metadata.title !== 'My Web Collection'))) {
                loadSessionData(parsed);
                showToast('Previous editing session restored.');
            }
        }
    } catch (e) {
        console.warn('Failed to load autosave from localStorage:', e);
    }
}
...
async function handleSaveSession() {
    ...
    if (!pythonApi) {
        ...
        a.download = `${sessionData.metadata.title || 'Session'}.bokasafnari`;
        a.click();
        URL.revokeObjectURL(a.href);
        showToast('Session saved to local file.');
        return;
    }

    try {
        const response = await pythonApi.save_session(sessionData);
        if (response.success) {
            showToast(`Session saved: ${response.filename}`);
        } else if (response.error !== 'Cancelled') {
            showToast(`Save session failed: ${response.error}`);
        }
    } catch (err) {
...
async function handleOpenSession() {
        ...
        input.onchange = (e) => {
            ...
            reader.onload = (evt) => {
                try {
                    ...
                    if (loadSessionData(data, file.name)) {
                        showToast(`Session loaded: ${file.name}`);
                    } else {
    ...
    try {
        const response = await pythonApi.open_session();
        if (response.success) {
            loadSessionData(response.data, response.filename);
        } else if (response.error !== 'Cancelled') {
            showToast(`Open session failed: ${response.error}`);
        }
    } catch (err) {
...
if (loadSessionBtn) loadSessionBtn.addEventListener('click', handleOpenSession);
...
function pushUndoState(description) {
    if (state.selectedChapterIndex !== null && state.chapters[state.selectedChapterIndex]) {
        // Ensure latest live DOM edits are synced before taking snapshot
        const clone = readerContent.cloneNode(true);
        clone.querySelectorAll('.edit-block-btn, .delete-block-btn').forEach(btn => btn.remove());
        const wrappers = Array.from(clone.querySelectorAll('.editable-block')).reverse();
        wrappers.forEach(wrapper => {
        ...
        state.chapters[state.selectedChapterIndex].content = content;
    }
...
function undo() {
    ...
    state.chapters = lastState.chapters;
    state.selectedChapterIndex = (typeof lastState.selectedChapterIndex === 'number' && lastState.selectedChapterIndex < state.chapters.length)
        ? lastState.selectedChapterIndex
        : (state.chapters.length > 0 ? 0 : null);
    ...
    showToast(`Undid: ${lastState.description}`);
}
